...
// import { Check, CheckCircle2, ChevronLeft, ChevronRight, Plus, Printer, GripVertical } from "lucide-react";
...
//                   <tr className="h-9">
//                     <th className="w-[6%] px-2 text-center text-[9px]">
//                       Drag
//                     </th>
...
//                         {/* Drag Handle */}
//                         <td className="w-6 px-4 text-center">
//                           <span title="Drag to change priority" className=" cursor-grab touch-none select-none items-center justify-center text-[#94a3b8] transition hover:text-[#075a91] active:cursor-grabbing">
//                             <span className="leading-[5px]"><GripVertical size={18} /></span>
//                           </span>
//                         </td>
...
export default function ApprovalWorkspace({
  ...
  const finalWorks = useMemo(() => {
    const workMap = new Map(
      availableWorks.map((work) => [work.id, work])
    );
  ...
  const toggleSelectAll = () => {
    if (allDisplayedSelected) {
      const displayedIds = new Set(
        displayedWorks.map((work) => work.id)
      );
  ...
  const handleDrop = (e: DragEvent<HTMLTableRowElement>, targetWorkId: string) => {
    e.preventDefault();
  ...
  const handleSubmit = () => {
    ...
    console.log(
      "Submitting works with priority:",
      worksWithPriority
    );
  };
  ...
  return (
    <div className="relative flex h-[calc(100dvh-128px)] min-h-0 w-full flex-col overflow-hidden bg-[#f3f7fa]">
      ...
      <div className="relative min-h-0 flex-1 overflow-hidden">
                ...
                <tbody>
                  ...
                  {finalWorks.length === 0 ? (
                    ...
                    finalWorks.map((work, index) => {
                      ...
                      const isDragTarget =
                        dragOverWorkId === work.id &&
                        draggedWorkId !== work.id;

                      return (

                        <tr key={`final-${work.id}`} draggable onDragStart={(e) => handleDragStart(e, work.id)} onDragOver={(e) => handleDragOver(e, work.id)} onDrop={(e) => handleDrop(e, work.id)} onDragEnd={handleDragEnd} className={`h-10 border-b border-[#e6eef3] transition-all ${draggedWorkId === work.id ? "bg-[#fff4e8] opacity-50" : dragOverWorkId === work.id ? "bg-[#eef6fb] shadow-[inset_0_2px_0_#f58220]" : "hover:bg-[#f5fafc]"}`}>

                          {/* DRAG HANDLE */}

                          <td className="w-[6%] px-2 text-center">
                            <div title="Drag to change priority" className="inline-flex cursor-grab touch-none select-none items-center justify-center rounded-md p-1 text-[#94a3b8] transition hover:bg-[#eef6fb] hover:text-[#075a91] active:cursor-grabbing">
        ...
        <div
          className={`absolute inset-y-0 left-0 z-[1000] w-1/2 p-2 transition-transform duration-300 ease-out ${leftDrawerOpen
            ? "translate-x-0"
            : "-translate-x-full"
            }`}
        >
        ...
        <div
          className={`absolute inset-y-0 right-0 z-[1000] w-1/2 p-2 transition-transform duration-300 ease-out ${rightDrawerOpen
            ? "translate-x-0"
            : "translate-x-full"
            }`}
        >
          ...
          <div className="flex h-full min-h-0 flex-col overflow-hidden rounded-[10px] border border-[#d5e2ea] bg-white shadow-[0_8px_30px_rgba(0,0,0,0.18)]">
            ...
            <div className="flex h-[52px] shrink-0 items-center justify-end border-t border-[#d5e2ea] bg-white px-3">
              ...
              <button
                ...
                className={`inline-flex h-8 items-center gap-1.5 rounded-[5px] px-4 text-[8px] font-extrabold transition ${finalWorks.length === 0
        ...
        <button
          ...
          className={`absolute left-0 top-1/2 z-[1100] flex h-16 w-4 -translate-y-1/2 cursor-pointer items-center justify-center rounded-r-[6px] bg-[#075a91] text-white shadow-lg transition-all duration-300 hover:bg-[#f58220] ${leftDrawerOpen
            ? "translate-x-[calc(50vw-24px)]"
            : "translate-x-0"
            }`}
        ...
        <button
          ...
          className={`absolute right-0 top-1/2 z-[1100] flex h-16 w-4 -translate-y-1/2 cursor-pointer items-center justify-center rounded-l-[6px] bg-[#075a91] text-white shadow-lg transition-all duration-300 hover:bg-[#f58220] ${rightDrawerOpen
            ? "-translate-x-[calc(50vw-24px)]"
            : "translate-x-0"
            }`}
      ...
      <div className="z-[1200] flex h-[52px] shrink-0 items-center justify-between border-t border-[#d5e2ea] bg-white px-3 shadow-[0_-3px_12px_rgba(0,59,99,0.08)] sm:h-[56px] sm:px-4">
        ...
        <div className="flex min-w-0 items-center gap-2">
          ...
          <span
            className={`h-2 w-2 shrink-0 rounded-full ${finalWorks.length > 0
              ? "bg-[#008f6a]"
              : "bg-[#f58220]"
              }`}
          />
          ...
          <div className="min-w-0">
            ...
            <p className="hidden text-[6px] text-slate-400 sm:block">
              {finalWorks.length > 0
                ? `${finalWorks.length} work${finalWorks.length > 1 ? "s" : ""
                } ready for approval`
        ...
        <button
          ...
          className={`flex h-8 shrink-0 items-center gap-1.5 rounded-[6px] px-4 text-[8px] font-extrabold transition sm:h-9 sm:px-5 sm:text-[9px] ${finalWorks.length === 0
            ? "cursor-not-allowed bg-slate-200 text-slate-400"
      ...
      <div
        id="approval-print-area"
        className="approval-print-only"
      >
