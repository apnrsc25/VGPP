...
//   type SortColumn = "theme" | "subTheme" | "workName" | "quantity" | "unit";
...
//   /*
//    * ---------------------------------------------------------
//    * EXPANDED CATEGORY / SUB CATEGORY STATE
//    * ---------------------------------------------------------
//    */
...
//       const category = work.theme?.trim() || "Other";
...
//     return sortedCategories.map(([categoryName, categoryWorks]) => {
//       const subCategoryMap = new Map<string, Work[]>();

//       categoryWorks.forEach((work) => {
//         const subCategory = work.subTheme?.trim() || "Other";

//         if (!subCategoryMap.has(subCategory)) {
//           subCategoryMap.set(subCategory, []);
//         }

//         subCategoryMap.get(subCategory)!.push(work);
//       });

//       const subCategories = Array.from(subCategoryMap.entries())
//         .sort(([subA], [subB]) =>
//           subA.localeCompare(subB, undefined, {
//             sensitivity: "base",
//           })
//         )
//         .map(([name, subWorks]) => ({
...
//             <th className="w-[36%] px-2 text-left">
...
//                 <tr onClick={() => toggleCategory(category.name)} className="h-11 cursor-pointer border-b border-[#cbdde8] bg-[#f3f9fc] transition hover:bg-[#e7f3f9]">
...
//                       <span className={`h-2 w-2 shrink-0 rounded-full ${themeClasses.dot}`} />

//                       <span className={`truncate text-[9px] font-extrabold uppercase tracking-[0.35px] sm:text-[10px] ${themeClasses.text}`}>
//                         {category.name}
//                       </span>
...
//                       return (
//                         <Fragment key={`subcategory-${subKey}`}>

//                           {/* -----------------------------------------
//                               SUB CATEGORY ROW
//                           ----------------------------------------- */}
//                           <tr onClick={() => toggleSubCategory(category.name, subCategory.name)} className="h-10 cursor-pointer border-b border-[#e1ebf0] bg-white transition hover:bg-[#f7fbfd]">
...
//                                 return (
//                                   <tr key={work.id} className="group h-10 border-b border-[#edf2f5] bg-white text-[#334155] transition hover:bg-[#f8fbfd] sm:h-11">
...
//                                       <button type="button" disabled={isDisabled} onClick={(event) => { event.stopPropagation(); onAdd(work); }} title={isDisabled ? "Already selected" : "Add work"} className={`mx-auto flex h-7 w-7 cursor-pointer items-center justify-center rounded-full border transition-all active:scale-90 ${isDisabled ? "cursor-not-allowed border-slate-200 bg-slate-50 text-slate-300" : "border-[#9ed1b8] bg-[#f0fbf5] text-[#00875a] hover:border-[#00875a] hover:bg-[#00875a] hover:text-white"}`}>
